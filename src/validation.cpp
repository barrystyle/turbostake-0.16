...
const std::string strMessageMagic = "TurboStake Signed Message:\n";
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // turbostake
            undo.nTime = alternate.nTime;           // turbostake
        } else {
...
void ThreadScriptCheck() {
    RenameThread("turbostake-scriptch");
...
bool TurboStakeContextualBlockChecks(const CBlock& block, CValidationState& state, CBlockIndex* pindex, bool fJustCheck)
{
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (pindex->nStakeModifier == 0 && pindex->nStakeModifierChecksum == 0 && !TurboStakeContextualBlockChecks(block, state, pindex, fJustCheck))
    ...
    // turbostake: coinbase reward check relocated to CheckBlock()
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // turbostake: Don't reject in case of old clients. Change our assumption instead.
        //trboTODO: Maybe add restrictions until when this is allowed? We don't want new clients to pretend to be old clients and try to abuse this.
    ...
    //trboTODO - move this somewhere in the upper calls, where pfrom is visible
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    // turbostake: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // turbostake: check PoS
    if (fCheckPoS && !TurboStakeContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool* fNewBlock, CBlockIndex** ppindex, bool* fPoSDuplicate)
{
    ...
    //trboTODO: move this somewhere else, because at this point we did not do some PoS checks that can only be done in ConnectBlock()
...
bool CChainState::LoadBlockIndex(const Consensus::Params& consensus_params, CBlockTreeDB& blocktree)
{
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight)
    {
        ...
        // turbostake: calculate stake modifier checksum
...
bool static LoadBlockIndexDB(const CChainParams& chainparams)
{
    ...
    // turbostake: load hashSyncCheckpoint
...
bool LoadBlockIndex(const CChainParams& chainparams)
{
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // turbostake: txindex is always enabled
