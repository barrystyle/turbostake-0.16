...
const QString BITCOIN_IPC_PREFIX("turbostake:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/turbostake-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/turbostake-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/turbostake-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the turbostake: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // turbostake: URI
        {
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click turbostake: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start turbostake: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling turbostake: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in turbostake: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // turbostake: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
